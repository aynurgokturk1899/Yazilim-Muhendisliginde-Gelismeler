# hasta_takip/doctor_client.py (SON VE KALICI ÇÖZÜM)
...
def index():
    ...
    try:
        ...
        approved_response = requests.get(f"{API_BASE_URL}/api/doctor/{doctor_id}/patients/approved")
        approved_response.raise_for_status()
        approved_patients = approved_response.json()
        
        # NOT: Geçici ekleme mantığı, API'den kalıcı liste çekildiği için artık gerekli değildir ve kaldırılmıştır.
