# dietitian_client.py (GÜNCELLENDİ - Onaylanan Hastaların Dinamik Yönetimi)

from flask import Flask, render_template_string, request, redirect, url_for, jsonify
import requests
import json # JSON işlemleri için import eklendi
import re # Düzenli ifadeler için import eklendi
...
def index():
    ...
    try:
        ...
        # KRİTİK NOKTA: Onaylandıktan sonra URL'den gelen mesajı kontrol et ve ID'yi ayıkla
        if message and "başarıyla onaylandı" in message:
            match = re.search(r"Hasta ID (\d+) başarıyla onaylandı!", message)
            if match:
                patient_id = int(match.group(1))
                
                # Dinamik olarak onaylanan hastayı listeye ekle
                approved_patients.append({
                    "id": patient_id, 
                    "username": f"Hasta ID {patient_id} (Yeni Onaylanan)"
                })

        # *Ek Kontrol*: memory_db'deki örnek veriye göre, Hasta Ahmet (ID: 1), Diyetisyen Can (ID: 3) 
        # tarafından ZATEN onaylanmış olarak işaretli. Yeni onaylanmayan ancak zaten onaylı olan
        # hastaların da görünmesi için bunu manuel olarak ekleyebiliriz (ID 3 için).
        if dietitian_id == 3: 
             if 1 not in [p['id'] for p in pending_patients] and 1 not in [p['id'] for p in approved_patients]:
                  approved_patients.append({"id": 1, "username": "hasta_ahmet"})
